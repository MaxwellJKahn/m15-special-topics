...
library(rpart.plot)
# Read in data
setwd('~/Desktop/Info/m15-special-topics/exercise-2')
...
for(i in 1:100) {
  ...
  indicies <- sample(nrow(homes), sample.size)
  training.data <- homes[indicies,]
  test.data <- homes[-indicies,]
  
  # 2. Pass your **training data** to the `rpart` function to run a simple classification operation
  
  simple.classification.operation <- rpart(in_sf ~ ., data = training.data, method="class")
  
  # 3. Pass your results to the `AssessFit` function to assess the fit
  
  my.assessment <- AssessFit(simple.classification.operation)
  
  # 4. Store your assessment in the `basic.fits` vector
  
  basic.fits[i] <- my.assessment
}
