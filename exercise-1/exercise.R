...
graphCountry <- function(input.country) {
toFind <- filter(gapminder, country == input.country)
toFind %>% 
  ggplot(aes(year, lifeExp)) + 
  geom_line() + 
  ggtitle("Full data = ")
model <- lm(lifeExp ~ year, data = toFind)
toFind %>% 
...
glance %>% 
  arrange(r.squared)
doesnt.fit.model <- filter(glance, r.squared < .25)
